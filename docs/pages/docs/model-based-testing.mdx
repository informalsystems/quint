...
We can create a folder called `traces` to hold all of our trace files an then generate as many as we want using `--n-traces`:
```sh
...
- `bank.qnt` is the file holding the model for which we want to generate traces from
- `--mbt` says we want extra info in the traces that are very useful for model based testing
  - `mbt::actionTaken` which will hold which action was used to reach the given state
