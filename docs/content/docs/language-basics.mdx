...
Actions named `init` and `step` are special as they define the **state machine**. Action `init` defines the initial states and `step` defines the transitions (you can use different names and specify them on the CLI, but `init` and `step` are the defaults). We use the prime operator to define them. Remember this example from [What does Quint do](./what-does-quint-do):
