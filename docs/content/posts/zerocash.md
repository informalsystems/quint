...
# Zerocash and the Fairie-Gold Attack
...
This approach was popularized by Zcash, whose design builds on the Zerocash protocol. Similar ideas appear in other systems, such as Namada and Penumbra. At a high level, Zerocash works as follows.
...
Our Quint specification simulates PRF, COMM, and zk-SNARKS. In contrast, it omits CRH, ENC, and Sig. This is because our main focus is to showcase the fairie-gold attack, and none of these three primitives are relevant for that.
...
To demonstrate the fairie-gold vulnerability, one simply needs to run the Quint simulator with the `faerie_gold_step` step, which interleaves honest actions (`mint` and `pour`) with the malicious one (`faerie_gold_pour`), and check the completeness invariant. 
```
...
```
quint run --invariant=completeness --step=faerie_gold_step_fixed zerocash.qnt
```
