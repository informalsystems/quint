...
- If it is set to `0`, no send operation is in progress. Then we just call the `frbc` operator, and update the process state. If the state is `Send(msg)`, we initiate the send to all by setting `send_to` to `1`.
