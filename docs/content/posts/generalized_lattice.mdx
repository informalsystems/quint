...
date: 2026-04-13
excerpt: "Proving liveness failures in Byzantine Generalized Lattice Agreement algorithm with Quint."
authors:
...
Simulation showed us that Bug 1 exists, and reasoning told us why it is permanent. But we wanted a machine-checked witness, a fully explored execution where the model checker exhausts all behaviours and confirms the liveness failure is unavoidable. We expressed the failure as a temporal liveness property: once a node has broadcast its round-1 value, it must eventually decide on the round-1 values. Both the buggy and correct specs use the same property:

```quint
temporal inclusivity_liveness_alg4 =
  (strongFair(step_n1, n1) and strongFair(step_n2, n2)) implies
  ((n1.val_sent.contains(1) leadsTo lat_le(R1_VALS, n1.decided_set))
    and (n2.val_sent.contains(1) leadsTo lat_le(R1_VALS, n2.decided_set)))
```

Here `val_sent.contains(1)` means "the node has broadcast its round-1 values" (1 is a round index, not a data value). `R1_VALS` is the set of values in the round-1 batch. Strong fairness is the right assumption for a liveness property like this: weak fairness is not enough, because a scheduler that briefly disables an action by modifying an unrelated variable can dodge the weak-fairness obligation indefinitely. The assumption does not hide Bug 1: by the time the deadlock takes hold, both nodes have already broadcast their round-1 values, so `val_sent.contains(1)` is genuinely true. In the deadlock state all actions are permanently disabled, so the strong fairness condition is satisfied vacuously, but the liveness goal (deciding `R1_VALS`) is never met. TLC sees this as a genuine counterexample and reports the violation.

The Choreo-based spec is a full-fidelity model of the protocol (it stores the complete message history to support rich simulation and state comparison). For exhaustive model checking we wanted a leaner target, so we wrote a minimal flat two-node spec that captures only the variables needed to express the property. TLC found a 27-step counterexample in 1 second, across 2119 distinct states checked in 2 fairness branches.
...
**States 1–17: Round 0 completes correctly.**
Both nodes RB-broadcast their round-0 value `{1}`, collect deliveries (`svs[r0] = {1}`, `proposed_set = {1}`), exchange `AckReq`s and `ACK`s, and decide `decided_set = {1}`. Both advance `safe_round` to 1. Round-0 work is complete.

**States 18–27: Round 1 —- the bug triggers.**
Both nodes advance to `proposer_round = 1` and RB-broadcast their round-1 value `{2}`. After collecting deliveries:
...
We added a second variant specification identical to the original except that the acceptor's `RespondAckReq` gate uses the correct `SAFE` predicate instead of `SAFE_A`. It uses the same temporal property.
...
The contrast is telling: under `SAFE_A`, TLC finds a genuine structural deadlock regardless of scheduling. Under the correct `SAFE`, TLC finds no violation at all. That is the confirmation we were looking for.
...
In this case, two bugs in the `SAFE` predicates were present in the original paper's pseudocode, both invisible in the single-round setting but breaking the protocol in different ways in round 1 and beyond. The first bug (in the acceptor's `AckReq` gate) caused a catastrophic liveness failure: every `AckReq` sent after round 0 is silently discarded, no quorum ever forms, and the algorithm makes no further progress. The second bug (in the proposer's `NACK` filter) is less severe but still causes nodes to stall in specific interleavings. Both were identified by reasoning: encoding the expected stuck state as a hypothesis invariant and running the simulator quickly produces a confirming trace, but both invariants also fire in correct executions: the real insight comes from reasoning about whether the `SAFE` check allows the stuck state to resolve. Formally proving Bug 1's liveness failure required a model checker: once we stripped away the Choreo framework overhead into a minimal flat spec, TLC exhausted the full reachable state space and produced a lasso counterexample: round 0 completes, both nodes advance and broadcast their round-1 values, and then the system freezes permanently. Both specs use the same temporal property with a strong-fairness assumption; the bug is still caught because by the time the deadlock takes hold, both nodes have already broadcast their round-1 values, making the liveness antecedent genuinely true. Swapping in the correct `SAFE` and re-running confirmed the fix: TLC exhausted the state space and found no violation.
