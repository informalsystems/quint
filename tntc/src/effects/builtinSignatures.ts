...
export function getSignatures (): Map<string, Signature> {
  ...
  const fixedAritySignatures = [literals, booleanOperators, setOperators, mapOperators, recordOperators, tupleOperators, listOperators, integerOperators, otherOperators].flat()
  fixedAritySignatures.forEach(sig => signatures.set(sig.name, (_) => parseEffectOrThrow(sig.effect)))
