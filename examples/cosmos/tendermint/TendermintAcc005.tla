...
\* @type: Set($proposeMsg);
AllFaultyProposals ==
  UNION { FaultyProposals(r): r \in Rounds }
...
\* @type: Set($preMsg);
AllFaultyPrevotes ==
  UNION { FaultyPrevotes(r): r \in Rounds }
...
\* @type: Set($preMsg);
AllFaultyPrecommits ==
  UNION { FaultyPrecommits(r): r \in Rounds }
...
Init ==
    ...
    /\ \E fproposals \in SUBSET AllFaultyProposals:
        msgsPropose = [ r \in Rounds |-> { m \in fproposals: m.round = r } ]
    /\ \E fprevotes \in SUBSET AllFaultyPrevotes:
        msgsPrevote = [ r \in Rounds |-> { m \in fprevotes: m.round = r } ]
    /\ \E fprecommits \in SUBSET AllFaultyPrecommits:
        msgsPrecommit = [ r \in Rounds |-> { m \in fprecommits: m.round = r } ]
    /\ evidencePropose = {}
    /\ evidencePrevote = {}
    /\ evidencePrecommit = {}
    /\ action = "Init"
