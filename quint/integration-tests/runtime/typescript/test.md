# TypeScript backend - test command tests

Integration tests for `quint test` with the TypeScript backend.
...
<!-- !test check Tendermint - Test -->
    quint test --backend=typescript --max-samples=100 --main TendermintModels ../examples/cosmos/tendermint/TendermintModels.qnt

### OK on test SimpleAuction.qnt

<!-- !test check SimpleAuction - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ../examples/solidity/SimpleAuction/SimpleAuction.qnt

### OK on test erc20.qnt::mempool

<!-- !test check erc20::mempool - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main=mempool ../examples/solidity/ERC20/erc20.qnt

### OK on test erc20.qnt::erc20Tests

<!-- !test check erc20::erc20Tests - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main=erc20Tests ../examples/solidity/ERC20/erc20.qnt

### OK on test ics20 bank

<!-- !test check ics20 bank - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main bankTests ../examples/cosmos/ics20/bank.qnt

### OK on test ics20 denomTrace
<!-- !test check ics20 denomTrace - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main=properChannelsTests ../examples/cosmos/ics20/denomTrace.qnt

### OK on test ics20
<!-- !test check ics20 - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main ics20Test ../examples/cosmos/ics20/ics20.qnt

### OK on test importFrom

<!-- !test check importFrom - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ../examples/language-features/importFrom.qnt

### OK on test basicSpells

<!-- !test check basicSpells - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ../examples/spells/basicSpells.qnt

### OK on test commonSpells

<!-- !test check commonSpells - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ../examples/spells/commonSpells.qnt

### OK on test rareSpells

<!-- !test check rareSpells - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ../examples/spells/rareSpells.qnt

### OK on test BoundedUint8

<!-- !test check BoundedUint8 - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript --main=BoundedUInt8Test ../examples/spells/BoundedUInt.qnt

### OK on test bug843pureValCache

<!-- !test check bug843pureValCache - Syntax/Types & Effects/Unit tests -->
    quint test --backend=typescript ./testFixture/bug843pureValCache.qnt

### OK on test simplePonzi

<!-- !test exit 0 -->
<!-- !test check simplePonzi - Test -->
    quint test --backend=typescript \
...
<!-- !test check test the default module -->
    quint test --backend=typescript ./testFixture/_1050diffName.qnt
...
<!-- !test in test runs -->
```
output=$(quint test --backend=typescript --main failingTestCounters --seed 1 ./testFixture/simulator/failingTestCounters.qnt)
exit_code=$?
...
```
output=$(quint test --backend=typescript --max-samples=10 --main TendermintModels ../examples/cosmos/tendermint/TendermintModels.qnt)
exit_code=$?
...
```
output=$(quint test --backend=typescript --seed 1 --verbosity=3 ./testFixture/simulator/failingTestCounters.qnt)
exit_code=$?
echo "$output" | sed -e 's/([0-9]*ms)/(duration)/g' -e 's#^.*failingTestCounters.qnt#      HOME/failingTestCounters.qnt#g'
exit $exit_code
```
...
<!-- !test in test itf -->
```
output=$(quint test --backend=typescript --out-itf='coin_{seq}_{test}.itf.json' \
...
<!-- !test in test1133 -->
```
quint test --backend=typescript --match='(t1|t2)' --out-itf='out_{seq}_{test}.itf.json' \
...
```
output=$(quint test --backend=typescript --seed=0x1286bf2e1dad07 --match=mintTwiceThenSendError \
...
```
quint test --backend=typescript --seed=NotANumber ../examples/tutorials/coin.qnt
```
...
```
output=$(quint test --backend=typescript testFixture/_1040compileError.qnt --seed=1 2>&1)
exit_code=$?
...
```
quint test --backend=typescript --main=invalid ./testFixture/_1050diffName.qnt
```
...
```
quint test --backend=typescript --out-itf {test}.itf.json ./testFixture/_1051manyTests.qnt >/dev/null
...
```
quint test --backend=typescript --out-itf {test}.itf.json ./testFixture/_1054sumTypesInItf.qnt >/dev/null
...
```
output=$(quint test --backend=typescript --seed=1 ./testFixture/thenErrorMessages.qnt)
exit_code=$?
...
```
quint test --backend=typescript --out-itf='{test}.itf.json' \
