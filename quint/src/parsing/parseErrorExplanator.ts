 ...
 * @param mkIdForLoc a handler that registers an error location and returns its id 
  ...
  const errorListener: any = {
    syntaxError: (_recognizer: any, offendingSymbol: any, line: number,
      ...
      if (exc instanceof LexerNoViableAltException) {
        ...
        errors.push({ code: 'QNT001', message: message, reference: id })
        return
      }
      if (exc instanceof NoViableAltException || exc instanceof InputMismatchException) {
        ...
        if (exc.getOffendingToken()) {
          ...
          const isEof = (input === '<EOF>')
          input = isEof ? 'end of file' : `'${input}'`
          const code = isEof ? 'QNT002' : 'QNT003'
          errors.push({ code, message: `unexpected ${input}`, reference: id })
          return
        }
