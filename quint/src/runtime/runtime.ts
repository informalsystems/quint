...
export type ComputableKind =
  'const' | 'var' | 'nextvar' | 'arg' | 'callable' | 'shadow'

/**
 * All available kinds as values.
 */
const allKinds: ComputableKind[] =
    ['const', 'var', 'nextvar', 'arg', 'callable', 'shadow']
...
export const kindName =
  (kind: ComputableKind, name: string | bigint): string => {
  return `${kind}:${name}`
}

export const getKind =
  (text: string): (ComputableKind | undefined) => {
  const prefix = text.split(':', 2)[0]
  return allKinds.find(k => k === prefix)
}
