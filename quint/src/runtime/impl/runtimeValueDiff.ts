...
import { RuntimeValue } from './runtimeValue' // adjust path if you paste into runtimeValue.ts itself
...
function prettyRVWith(v: RuntimeValue, c: (s: string) => string): Doc {
  // Bool
  try { return tint(String(v.toBool()), c) } catch {}

  // Int
  try { return tint(v.toInt().toString(), c) } catch {}

  // Str
  try { return group([tint('"', c), tint(v.toStr(), c), tint('"', c)]) } catch {}
  ...
  try {
    ...
    return isEmptyTuple
      ? labelDoc
      : group([labelDoc, nary(c, tint('(', c), [prettyRVWith(value, c)], tint(')', c))])
  } catch {}
  ...
  // Fallback
  try { return tint(JSONbig.stringify(v), c) } catch { return tint('<value>', c) }
}
...
export function diffRuntimeValueDoc(a: RuntimeValue, b: RuntimeValue): Doc {
  ...
  if ((a as any).equals?.(b)) return grayRV(a)

  // Primitive old => new
  const atomOld =
    (() => { try { return tint(String(a.toBool()), RED) } catch {}
             try { return tint(a.toInt().toString(), RED) } catch {}
             try { return group([tint('"', RED), tint(a.toStr(), RED), tint('"', RED)]) } catch {}
             return undefined })()
  const atomNew =
    (() => { try { return tint(String(b.toBool()), GREEN) } catch {}
             try { return tint(b.toInt().toString(), GREEN) } catch {}
             try { return group([tint('"', GREEN), tint(b.toStr(), GREEN), tint('"', GREEN)]) } catch {}
             return undefined })()
  ...
  try {
    ...
    Array.from(keys).sort().forEach(k => {
      const inA = ra.has(k), inB = rb.has(k)
      if (inA && inB) {
        fields.push(group([tint(k, GRAY), tint(':', GRAY), nest('  ', [line(), diffRuntimeValueDoc(ra.get(k)!, rb.get(k)!)])]))
      } else if (inA) {
  ...
  try {
    const ma = a.toMap(), mb = b.toMap()
    const aKeys = ma.keySeq().toArray(), bKeys = mb.keySeq().toArray()
  ...
  try {
    ...
    const payloadIsEmptyTuple = (rv: RuntimeValue): boolean => {
      try { return rv.toList().toArray().length === 0 } catch { return false }
    }
