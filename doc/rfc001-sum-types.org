...
*** Declaration
The proposed syntax fits closely with our existing syntax for records
...
These very flexible sum types are powerful, but they introduce challenges to the
syntax (and semantics) of programs. Fir example, supporting anonymous variant
types requires way of constructing variants without pre-defined constructors.
...
The currently proposed syntax for declaration is indexing pretty heavily on
current popularity of rust. More than I would prefer, since I worry that the
concrete syntax for declarations doesn't differentiate them enough from record
declarations.

This similarity is aggravated by the fact that we are currently planning to
allow sum type labels to be upper or lower case, just as we do for records.
Thus, we could end up with declarations like:
...
I expect that the difference between =_:_= and =_(_)= will generally be enough
to keep readers from confusing the two. But the chance of mistaking a record and
sum type declaration is actually compounding a worse possibli confusion: The
part enclosed in brackets is syntactically indistinguishable from a block of
operators.

Given that we tend to read data structures from the outside in, I'd feel more
confident that we were going to avoid confusion if declarations used the =|= as
...
The latter seems much clearer to me, and if we reflect this syntax also in
...
If we just adopted this without also changing our syntax for records, we would
introduce

- Breaks with our current convention around type declarations and
  keywords, unless we also redesign the syntax for records.
...
The proposed syntax is very close to [[https://doc.rust-lang.org/book/ch18-03-pattern-syntax.html#matching-literals][Rust's pattern syntax]]. Here's some example
Rust for comparison
...
Until we have pattern matching untroduced, we flag a parsing error of the
deconstructor arg is not a free variable, and inform the user that full pattern
